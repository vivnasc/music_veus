...
import { VENNA_ALBUMS } from "@/data/venna-albums";
    ...
    <div className="rounded-xl border border-mundo-muted-dark/20 bg-mundo-bg-light/50 p-4">
      ...
      <div className="flex items-start justify-between gap-3 mb-3">
        <div className="flex items-center gap-3 min-w-0">
          <span className="shrink-0 w-8 h-8 rounded-full bg-orange-900/40 text-orange-300 flex items-center justify-center text-xs font-bold">
            {track.number}
          </span>
      ...
      <div className="flex flex-wrap gap-2 mb-2 text-[11px]">
        <button onClick={() => setShowTitle(!showTitle)} className="text-orange-300 hover:text-orange-200">
          {showTitle ? "Esconder TITLE" : "Editar TITLE"}{titleEdited && <span className="text-amber-400 ml-1">●</span>}
        </button>
        <button onClick={() => setShowStyle(!showStyle)} className="text-orange-300 hover:text-orange-200">
          {showStyle ? "Esconder STYLE" : "Editar STYLE"}{styleEdited && <span className="text-amber-400 ml-1">●</span>}
        </button>
        <button onClick={() => setShowLyrics(!showLyrics)} className="text-orange-300 hover:text-orange-200">
      ...
      {showTitle && (
        <div className="mb-3 rounded-lg bg-black/30 p-3">
          ...
          <input
            ...
            className="w-full bg-mundo-bg p-2 rounded text-xs text-mundo-creme/90 font-mono border border-mundo-muted-dark/20 focus:border-orange-500 focus:outline-none"
          />
        </div>
      )}
      ...
      {showStyle && (
        <div className="mb-3 rounded-lg bg-black/30 p-3">
          ...
          <textarea
            ...
            className="w-full bg-mundo-bg p-2 rounded text-[11px] text-mundo-creme/90 font-mono border border-mundo-muted-dark/20 focus:border-orange-500 focus:outline-none min-h-[5rem] max-h-72 resize-y"
            spellCheck={false}
          />
      ...
      {showLyrics && (
        <div className="mb-3 rounded-lg bg-black/30 p-3">
          ...
          <textarea
            ...
            className="w-full bg-mundo-bg p-2 rounded text-[11px] text-mundo-creme/90 font-mono leading-relaxed border border-mundo-muted-dark/20 focus:border-orange-500 focus:outline-none min-h-[16rem] max-h-[32rem] resize-y"
            spellCheck={false}
          />
      ...
      <button
        ...
        className={`w-full rounded-lg px-4 py-2.5 text-xs font-medium transition mb-3 ${
          isWorking
            ? "bg-orange-900/20 text-orange-600 animate-pulse cursor-wait"
            : "bg-orange-700/30 text-orange-200 hover:bg-orange-700/50"
        }`}
      >
...
const EDITS_LS_KEY = "venna_admin_edits_v1";
...
export default function NovaAdminPage() {
  ...
  const [openAlbum, setOpenAlbum] = useState<string | null>(VENNA_ALBUMS[0]?.slug ?? null);
  ...
  const totalTracks = useMemo(
    () => VENNA_ALBUMS.reduce((s, a) => s + a.tracks.length, 0),
    []
  );
  ...
  useEffect(() => {
    ...
    async function checkExisting() {
      ...
      for (const album of VENNA_ALBUMS) {
  ...
  async function downloadDistrokidZip(album: Album, btn: HTMLButtonElement) {
    ...
    try {
      ...
      await downloadAlbumForDistribution(
        ...
        undefined, // sem custom title map; usa título original do EP
        coverTrackNum,
      );
  ...
  const filteredAlbums = useMemo(() => {
    if (!search.trim()) return VENNA_ALBUMS;
  ...
  return (
    <div className="min-h-screen bg-[#0D0D1A] text-[#F5F0E6] px-4 py-6 max-w-4xl mx-auto">
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-2xl font-serif text-orange-300 mb-1">VENNA</h1>
        <p className="text-xs text-mundo-muted">
          EP Mango Hour — 5 faixas pop dançáveis, melodic house, PT/EN com sotaque de Lisboa
        </p>
      ...
      <div className="flex gap-2 flex-wrap mb-6">
        ...
        <Link
          href="/admin/nova"
          className="shrink-0 rounded-lg bg-violet-900/20 px-4 py-2 text-xs text-violet-300 hover:bg-violet-900/40 transition"
        >
          NOVA
        </Link>
        <Link
        ...
        <select
          ...
          className="rounded-lg bg-orange-900/20 px-3 py-2 text-xs text-orange-300 border border-orange-800/30"
        >
        ...
        <input
          ...
          className="rounded-lg bg-black/30 border border-mundo-muted-dark/20 px-3 py-2 text-xs text-mundo-creme placeholder:text-mundo-muted-dark outline-none focus:border-orange-700/50 min-w-[180px]"
        />
      </div>

      {/* Como usar */}
      <div className="mb-6 rounded-xl border border-orange-900/30 bg-orange-950/20 p-4">
        <p className="text-[11px] text-orange-300 leading-relaxed">
          Cada faixa envia ao Suno o STYLE verbatim (campo Style of Music), as LYRICS verbatim, e o TITLE em modo Custom com letra. &ldquo;Honey Hour&rdquo; é a Persona Seed Track (já gerada e aprovada): cria a Persona a partir dela no Suno, cola aqui o Persona ID, e usa-a nas restantes 4 faixas para coerência vocal.
        </p>
      </div>
      ...
      <div className="mb-6">
        <input
          ...
          className="w-full rounded-lg bg-black/30 border border-mundo-muted-dark/20 px-4 py-2.5 text-sm text-mundo-creme placeholder:text-mundo-muted-dark outline-none focus:border-orange-700/50"
        />
      </div>
